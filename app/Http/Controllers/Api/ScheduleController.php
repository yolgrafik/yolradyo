...
class ScheduleController extends Controller
{
    public function __invoke(Request $request): JsonResponse
    {
        ...
        if ($liveDj) {
            // Öncelik 1: Canlı DJ varsa host eşleşmesine göre is_live
            $liveName = trim(mb_strtolower($liveDj->name));
            $items = collect($schedulesArray)->map(function ($s, $idx) use ($liveName) {
                ...
                $hostNorm = trim(mb_strtolower($s->host ?? ''));
                $isLive = $hostNorm !== '' && $hostNorm === $liveName;
