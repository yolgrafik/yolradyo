...
<div class="card" style="max-width:700px;">
    <div class="card-header">Yeni Slider</div>
    <div class="card-body">
        <form method="POST" action="{{ route('admin.sliders.store') }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" name="subtitle" id="subtitle" value="{{ old('subtitle') }}" class="form-input">
                @error('subtitle')<span class="form-error">{{ $message }}</span>@enderror
            </div>
            <div class="form-group">
                <label for="button_text">Buton Metni</label>
                <input type="text" name="button_text" id="button_text" value="{{ old('button_text') }}" class="form-input" placeholder="Daha Fazla">
                @error('button_text')<span class="form-error">{{ $message }}</span>@enderror
            </div>
            <div class="form-group">
                <label for="button_link">Buton Linki</label>
                <input type="text" name="button_link" id="button_link" value="{{ old('button_link') }}" class="form-input" placeholder="/programlar">
                @error('button_link')<span class="form-error">{{ $message }}</span>@enderror
            </div>
            <div class="form-group">
                <label for="image">Gorsel * (1920x600 onerilir)</label>
                <input type="file" name="image" id="image" required accept="image/*" class="form-input">
