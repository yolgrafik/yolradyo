        ...
        <form method="POST" action="{{ route('admin.members.update', $user) }}">
            ...
            <div class="form-group">
                ...
                <select name="status" id="status" class="form-input">
                    <option value="pending" {{ old('status', $user->status) === 'pending' ? 'selected' : '' }}>Beklemede</option>
                    <option value="approved" {{ old('status', $user->status) === 'approved' ? 'selected' : '' }}>Onaylı</option>
                    <option value="rejected" {{ old('status', $user->status) === 'rejected' ? 'selected' : '' }}>Reddedildi</option>
                </select>
                @error('status')<span class="form-error">{{ $message }}</span>@enderror
            </div>
            <div class="form-group">
                <label for="password">Yeni Şifre (boş bırakın değiştirmemek için)</label>
